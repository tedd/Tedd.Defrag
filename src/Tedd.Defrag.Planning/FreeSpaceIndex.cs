...
public sealed class FreeSpaceIndex : IDisposable
{
    ...
    public long AvailableFrom(long start)
    {
        int n = _root, candidate = 0;
        while (n != 0) { if (_nodes[n].Start <= start) { candidate = n; n = _nodes[n].Right; } else n = _nodes[n].Left; }
        return candidate == 0 ? 0 : Math.Max(0, _nodes[candidate].Start + _nodes[candidate].Length - start);
    }
