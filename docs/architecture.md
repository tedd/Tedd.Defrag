...
Try-all starts only from an uncompressed file. Each file tests XPRESS 4K, XPRESS 8K, XPRESS 16K and then LZX sequentially, removing intermediate backing between trials. If LZX is smallest, its final trial is retained directly; otherwise it is removed and the earlier winner is reapplied. An already compressed file is intentionally left unchanged. Different files use the resource policy's bounded compression-worker count, while progress publication and control checkpoints remain serialized. Preview performs scope enumeration without native mutation. Per-file failures are bounded in the warnings and produce a partial result while remaining targets continue. Compression rewrites are not governed by the relocation-byte pacer because each WOF operation is a synchronous, opaque filesystem request.

After each NTFS layout scan, streams marked compressed or reparse-backed are checked through `WofIsExternalFile` and `GetCompressedFileSizeW`. The snapshot records totals for all identified compressed files and retains the 500 entries with the largest byte savings for dashboard display. Ordinary NTFS compression, WOF file-provider algorithms and WIM backing are reported distinctly. Non-filesystem reparse points and inaccessible candidates remain outside the inventory.
