...
{
    private static VolumeInfo Info => new("refs-fixture", @"V:\", "fixture", "ReFS", 64 * 4096, 40 * 4096, 4096, true, true, [], "fixture");
    ...
    [Fact]
    public void CapabilityMatrixPreservesNtfsAndLimitsRefsToAnalysisAndWindowsMaintenance()
    {
        foreach (var operation in Enum.GetValues<Operation>())
        {
            Assert.True(FileSystemCapabilities.Supports("ntfs", operation));
            Assert.Equal(operation is Operation.Analyze or Operation.ReTrim or Operation.Automatic or Operation.SlabConsolidate or Operation.WindowsDefrag,
                FileSystemCapabilities.Supports("rEfS", operation));
            Assert.False(FileSystemCapabilities.Supports("exFAT", operation));
        }
        Assert.False(FileSystemCapabilities.Supports("NTFS", (Operation)999));
    }

    [Fact]
    public void RefsAnalysisPublishesPartialCoverageWithoutEligibleMovesOrNtfsMetadata()
    {
    ...
    public void RefsCustomOperationsFailBeforeScanningOrMoving(Operation operation)
    {
        var volume = new TestVolume();
        var result = Run(volume, Request(operation, true));
        Assert.Equal(JobState.Failed, result.State);
        Assert.Contains("requires NTFS", result.Message);
    ...
    public void RefsMaintenancePreviewNeverExecutesAndDisclosesCommand(Operation operation, string flag)
    {
        bool executed = false;
    ...
    public void RefsMaintenanceExecutesAndRescans(Operation operation)
    {
        var volume = new TestVolume();
        int executed = 0;
    ...
    [Fact]
    public void RefsTrimDoesNotDependOnAllocationScanAvailability()
    {
    ...
    [Fact]
    public void RefsAnalysisDoesNotSuppressScanFailure()
    {
    ...
    [Fact]
    public void RefsRecommendationsNeverRequestNtfsRelocation()
    {
    ...
    public void ExtentNormalizationMergesAdjacentRunsAndPreservesHoles()
    {
        Assert.Equal([new Extent(0, 10, 5), new(5, -1, 3), new(8, 15, 1)],
            RefsScanner.NormalizeExtents([new(0, 10, 2), new(2, 12, 3), new(5, -1, 3), new(8, 15, 1)], 20));
        Assert.Throws<IOException>(() => RefsScanner.NormalizeExtents([new(0, 19, 2)], 20));
        Assert.Throws<IOException>(() => RefsScanner.NormalizeExtents([new(1, 1, 2)], 20));
        Assert.Throws<IOException>(() => RefsScanner.NormalizeExtents([new(0, -2, 2)], 20));
    }
    ...
    private static JobSnapshot Run(TestVolume volume, JobRequest request, Action? maintenance = null)
    {
        string root = Path.Combine(Path.GetTempPath(), "Tedd.Defrag.RefsTests", Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        public VolumeInfo VolumeInfo { get; init; } = RefsTests.Info;
        public VolumeInfo Info => VolumeInfo;
        public int Scans { get; private set; }
        public Exception? ScanError { get; init; }
        public bool IsDirty() => throw new InvalidOperationException("Must not query NTFS health on ReFS");
        public VolumeLayout Scan(JobRequest request, Action<double, long, string> progress, Action checkpoint, CancellationToken token, Action<WorkProgress>? diagnostics = null)
        {
            ...
            return new(Info, 64, bitmap, [file], DateTimeOffset.UtcNow, 1, 0, false, ["Partial ReFS file coverage"]);
        }
        public byte[] ReadBitmap(JobRequest request, Action checkpoint, CancellationToken token) => throw new InvalidOperationException("No custom moves on ReFS");
        public void ExecuteMove(FileLayout file, PlannedMove move, PathRules rules) => throw new InvalidOperationException("No custom moves on ReFS");
        public void Dispose() { }
    }
}
